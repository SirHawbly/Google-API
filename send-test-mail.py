...
# List of all mimetype per extension: http://help.dottoro.com/lapuadlp.php  
	# or http://mime.ritey.com/
...
#   the one created and downloaded from 
# 		https://console.developers.google.com/apis/ (let's call it the client_id) 
#   the one that will be created from the downloaded client_id (let's call 
# 		it credentials, it will be store in C:\Users\user\.credentials)


        #Getting the CLIENT_ID 
            # 1) enable the api you need on 
						#				https://console.developers.google.com/apis/
            # 2) download the .json file (this is the CLIENT_ID)
            # 3) save the CLIENT_ID in same folder as your script.py 
            # 4) update the CLIENT_SECRET_FILE (in the code below) 
						# 			with the CLIENT_ID filename


        #Optional
        # If you don't change the permission ("scope"): 
            # the CLIENT_ID could be deleted after creating the credential 
						# (after the first run)

        # If you need to change the scope:
            # you will need the CLIENT_ID each time to create a new credential 
...
def create_message_and_send(sender, to, subject,  message_text_plain, message_text_html, attached_file):
    ...
    # message_without_attachment = create_message_without_attachment(sender, 
			# to, subject, message_text_html, message_text_plain)
    # send_Message_without_attachement(service, "me", 
			# message_without_attachment, message_text_plain)
